...
public class SubsetsWithDup {
    ...
    public List<List<Integer>> solutionV2(int[] nums){
        ...
        dfsV2(nums, visited, 0, n, 0, path, ans);

        return ans;
    }

    private void dfsV2(int[] nums, boolean[] visited, int begin, int k, int depth, Deque<Integer> path, List<List<Integer>> res){
        ...
        for (int i = begin; i < k; i++){
            ...
            path.addLast(nums[i]);
            visited[i] = true;
            dfsV2(nums, visited, i + 1, k, depth + 1, path, res);
            path.removeLast();
            visited[i] = false;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> lists = subsetsWithDup.solutionV2(nums);
