package com.leetcode.www.middle.array;

import java.util.HashMap;
import java.util.Map;

/**
 * leetcode-904:水果成篮
 * 你正在探访一家农场，农场从左到右种植了一排果树。这些树用一个整数数组 fruits 表示，其中 fruits[i] 是第 i 棵树上的水果 种类 。
     * 你想要尽可能多地收集水果。然而，农场的主人设定了一些严格的规矩，你必须按照要求采摘水果：
     * 你只有 两个 篮子，并且每个篮子只能装 单一类型 的水果。每个篮子能够装的水果总量没有限制。
     * 你可以选择任意一棵树开始采摘，你必须从 每棵 树（包括开始采摘的树）上 恰好摘一个水果 。采摘的水果应当符合篮子中的水果类型。每采摘一次，你将会向右移动到下一棵树，并继续采摘。
       一旦你走到某棵树前，但水果不符合篮子的水果类型，那么就必须停止采摘。
     * 给你一个整数数组 fruits ，返回你可以收集的水果的 最大 数目。
 */
public class TotalFruit {


    /**
     * 滑动窗口
     * 由left和right指针构成一个滑动窗口，right指针向右移动，再使用一个哈希表统计这个滑动窗口内每种类型的水果的数量，当窗口内的水果类型大于2种时，
     * 就移动左指针，直到窗口内的水果类型不大于2种为止。在right指针向右移动的过程中同时计算窗口的大小，窗口大小的最大值就是答案
     * 复杂度分析
     *  时间复杂度:O(n),n是数组fruits的长度
     *  空间复杂度:O(n),n是数组fruits的长度，哈希表所使用的空间
     * @param fruits
     * @return
     */
    public int solution(int[] fruits){

        if (fruits.length == 0){
            return 0;
        }

        int ans = 0;
        Map<Integer, Integer> cntMap = new HashMap<>();
        int left = 0;
        for (int right = 0; right < fruits.length; right++){
            cntMap.put(fruits[right], cntMap.getOrDefault(fruits[right], 0) + 1);
            while (cntMap.size() >= 3){
                cntMap.put(fruits[left], cntMap.get(fruits[left]) - 1);
                if (cntMap.get(fruits[left]) == 0){
                    cntMap.remove(fruits[left]);
                }
                ++left;
            }
            ans = Math.max(ans, right - left + 1);
        }

        return ans;
    }

    public static void main(String[] args) {

        TotalFruit fruit = new TotalFruit();
        int[] fruits = {3,3,3,1,2,1,1,2,3,3,4};
        int ans = fruit.solution(fruits);
        System.out.println(ans);
    }
}
